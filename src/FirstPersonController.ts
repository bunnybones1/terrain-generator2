...
export default class FirstPersonController {
  ...
  constructor(
    ...
    const groundH0 = terrainSampler.getSample(spawn.x, spawn.z).height + initialHeight;
  ...
  update(dt: number) {
    ...
    if (this.isFlying) {
      ...
      const groundH_fly = this.terrainSampler.getSample(this.target.x, this.target.z).height;
      ...
      const groundH_swim = this.terrainSampler.getSample(this.target.x, this.target.z).height;
      ...
      const groundH = this.terrainSampler.getSample(this.target.x, this.target.z).height;

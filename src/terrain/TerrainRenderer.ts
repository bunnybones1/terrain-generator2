...
export class TerrainRenderer {
  ...
  private buildTile(t: TileCoords): Mesh {
    ...
    const uvs: number[] = [];
    const pines: number[] = [];
    const invAO: number[] = []; //inverted Ambient Occlusion (1 is dark, 0 is bright)
    ...
    for (let j = 0; j < res; j++) {
      for (let i = 0; i < res; i++) {
        ...
        invAO.push(useBaseForHighLod ? terrainSample.pineWindow : 0);
    ...
    geo.setAttribute("pine", new BufferAttribute(new Float32Array(pines), 1));
    geo.setAttribute("invAO", new BufferAttribute(new Float32Array(invAO), 1));
    geo.setIndex(indices);
